...
def edit_profile(request):
    """Displays the user profile editing form on a GET request, on a POST validates and saves the user's information.
    Must be logged in to edit one's profile.

    :param request: Request data provided by the WSGI
    :return: Rendered template for editing user template on GET or unsuccessful validation
...
@csrf_exempt
def login(request):
    """Displays the login form on a GET request, otherwise validates and signs in the user on a POST

    :param request: Request data provided by the WSGI
    :return: Rendered template displaying the login form on a GET or unsuccessful validation,
             else redirects to given `next` parameter or website index
    """
    if request.method == 'GET':
        form = LoginForm()
...
@csrf_exempt
def signup(request):
    """Displays the signup form on a GET, otherwise redirects to the home page on a successful signup.

    :param request: Request data provided by the WSGI
    :return: Rendered template displaying the signup form on a GET or unsuccessful validation,
             else redirects to the homepage
    """
...
def questions(request):
    """Displays the form to post a question or redirects to the question's thread once it has been successfully created
...
def answers(request, id_):
    """Manages the different actions that occur when interacting and displaying a question:
...
def vote(request):
    ...
    :param request: Request data provided by the WSGI
    :return: JSONResponse with the post's `new_score` and the post's score tag `id` in the DOM tree
    """
...
class QuestionDisplayView(ListView):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        pagination_data = self._pagination_data(paginator_all_questions, page_obj, context['is_paginated'])
    ...
    def _pagination_data(self, paginator, page, is_paginated):
        ...
        if page_number == 1:
            ...
            # Last page
            pages_on_left = page_range[(page_number - 3) if (page_number - 3) > 0 else 0:page_number - 1]
...
def edit(request, id_):
    """Displays question editing form on a GET and modifies the question on a successful validation.
